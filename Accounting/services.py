...
from Accounting.models import Account, Balance, JournalEntry, Transaction
...
def account_transfer(data):
    ...
    try:
        if data.je.amount == 0:
            return status.HTTP_400_BAD_REQUEST, {'detail', 'amount should be more than zero'}
        ...
        cje = JournalEntry.objects.create(
            account_id = data.je.credit_account,
            transaction = t,
        ...
        dje = JournalEntry.objects.create(
            account_id = data.je.debit_account,
            transaction = t,
